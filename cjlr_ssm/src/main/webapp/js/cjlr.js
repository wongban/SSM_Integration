...
$(document).ready(function() {
  ...
  xmlhttp1.open("GET", "cjlr/queryJsonXnxq?time=" + new Date().getTime(), true);
  xmlhttp1.send();
  ...
  xmlhttp2.open("GET", "cjlr/queryJsonCjbz?time=" + new Date().getTime(), true);
  xmlhttp2.send();
  ...
  xmlhttp3.open("GET", "cjlr/queryJsonCjdj?time=" + new Date().getTime(), true);
  xmlhttp3.send();
});
...
function operate() {
  ...
  case "exportButton":
    ...
    var url = "cjlr/exportCjjl?rwbh=#rwbh&part=#part&time=#time".
  ...
  case "importButton":
    ...
    $(form).ajaxSubmit({
      url: "cjlr/importCjjl?time=" + new Date().getTime(),
      type: "POST",
      dataType: "json",
...
function save(action) {
  ...
  xmlhttp.open("POST", "cjlr/"+ action + "?time=" + new Date().getTime(), true);
  xmlhttp.setRequestHeader("Content-type", "application/json");
  xmlhttp.send(JSON.stringify(lrrw)); // 将成绩记录数据JSON化，并以POST的形式提交
}
...
function autoSave() {
  intervalID = setInterval(function() {
    ...
    xmlhttp.open("POST", "cjlr/save?time=" + new Date().getTime(), true);
...
function loadLrrw() {
  ...
  xmlhttp.open("GET", "cjlr/queryJsonLrrw?dqxnxq=" + dqxnxq + "&time=" + new Date().getTime(), true);
  xmlhttp.send();
}
...
function loadCjjl(rwbh, queryBean) {
  ...
  xmlhttp.open("POST", "cjlr/queryJsonCjjl?cjlrrwh=" + rwbh + "&time=" + new Date().getTime(), true);
  xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  if (queryBean != null) {
